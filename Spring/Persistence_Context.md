...
쓰기 지연 저장소는 엔티티에 변경이 일어나는 쿼리를 저장하는 공간입니다.

트랜잭션 안에서 엔티티에 대한 변경이 일어날 때마다 쓰기 지연 저장소에 해당 쿼리를 저장하고 트랜잭션이 커밋되는 순간 한 번에 DB에 전송합니다.
...
![First Level Cache](/images/first_level_cache.png)

1차 캐시에는 "스냅샷"이라는 것이 존재합니다.

스냅샷은 엔티티가 1차 캐시에 저장될 때, 저장되는 시점의 상태를 스냅샷으로 만들어 1차 캐시에 보관합니다.


**트랜잭션이 커밋되는 시점**에 **엔티티와 스냅샷을 비교**하여 서로 차이가 있을 경우 JPA가 이를 감지하여 DB에 반영합니다.

따라서 커밋되기 전에 엔티티를 수정하여 사용하였다가, 커밋되는 시점에 다시 원상 복구 시키면 update 쿼리가 실행되지 않습니다.
